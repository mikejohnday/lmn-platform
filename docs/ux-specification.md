...
- **Submission type selector must appear first.** It drives all Step 2 conditional logic. Radio buttons, large tap targets. ⚠️ **"Vertically stacked" is superseded on desktop by `amendments.md` AM-07** — the built layout renders the two cards side by side at ≥481px, stacking to a single column on mobile only. The wording here is left on file per this document's convention of not rewriting superseded text; AM-07 is authoritative. ✅ **`Next` remains clickable when no type is selected**; clicking it shows the approved error `Please select a submission type.` below the radio group and does not advance. `[AM-04 — supersedes KAN-20 §2.1's "must be selected before Next is enabled"]`
